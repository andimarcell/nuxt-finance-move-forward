...
const categorySummary = computed(() => {
  ...
  const filtered = props.transactions.filter(t => t.type?.toLowerCase() === chartType.value)
  ...
  const list = Object.keys(summary).map((cat, index) => {
    const amount = summary[cat]
    const percent = totalAmount === 0 ? 0 : Math.round((amount / totalAmount) * 100)
        ...
        donut: {
          ...
          labels: {
            ...
            value: {
              ...
              offsetY: 6,
              formatter: () => {
                // Tampilkan persentase kategori terbesar di tengah lingkaran
                if (categorySummary.value.list.length === 0) return "0%"
                return categorySummary.value.list[0].percent
              }
            },
            total: {
              show: true,
              label: categorySummary.value.list.length > 0 ? categorySummary.value.list[0].name : "Nihil",
              color: "#9ca3af",
              formatter: () => {
                if (categorySummary.value.list.length === 0) return "0%"
                return categorySummary.value.list[0].percent
...
<template>
  <div class="p-6 bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl shadow-sm flex flex-col justify-between h-full">
    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex gap-1 bg-gray-50 dark:bg-gray-800 p-1 rounded-lg border border-gray-100 dark:border-gray-800 shrink-0">
        <button
          type="button"
          class="px-2.5 py-1 text-[10px] font-black rounded-md transition duration-150 cursor-pointer"
          :class="chartType === 'expense' ? 'bg-primary-500 text-white shadow-sm' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'"
        ...
        <button
          type="button"
          class="px-2.5 py-1 text-[10px] font-black rounded-md transition duration-150 cursor-pointer"
          :class="chartType === 'income' ? 'bg-primary-500 text-white shadow-sm' : 'text-gray-500 hover:text-gray-900 dark:hover:text-white'"
    ...
    <div v-if="categorySummary.list.length > 0" class="space-y-5">
      <div class="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest text-center">
        Pecahan Kategori
      </div>

      <!-- Kontainer List: Berjejer vertikal rapi di tengah -->
      <div class="grid grid-cols-2 gap-x-4 gap-y-5 justify-items-center max-w-sm mx-auto">
        <div 
        ...
        >
          ...
          <div class="flex items-center space-x-2">
            ...
            <span class="text-xs font-bold text-gray-900 dark:text-white truncate max-w-22.5 capitalize">
              {{ item.name }}
            </span>
          </div>
    ...
    <div class="mt-8 pt-4 border-t border-gray-100 dark:border-gray-800 text-center">
      ...
      <div class="flex items-center justify-center space-x-1.5 text-[10px] text-gray-400 dark:text-gray-500 font-bold uppercase tracking-wider">
